function lfs = iQFM_multi_echo(phase, TE, varargin)
...
if imsize(4) > 1
    for echo_num = 1 : imsize(4)
        ...
        tmp_chi = chi(:,:,:,echo_num);
        tmp_chi = imresize3(tmp_chi,imsize(1:3));
        tmp_chi = TE(echo_num) .* tmp_chi;
%         chi(:,:,:,echo_num) = imresize3(chi(:,:,:,echo_num),imsize(1:3));
%         chi(:,:,:,echo_num) = TE(echo_num) .* chi(:,:,:,echo_num);
        chi2(:,:,:,echo_num) = tmp_chi;

        tmp_mag = mag(:,:,:,echo_num);
        tmp_mag = imresize3(tmp_mag,imsize(1:3));
        mag2(:,:,:,echo_num) = tmp_mag;
    end
    ...
    nii = make_nii(chi * (2.675e8 * B0 * 1e-6), vox);
    save_nii(nii, [output_dir,'/iQFM_multi_echo.nii']);
